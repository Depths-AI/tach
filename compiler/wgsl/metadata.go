...
func describePlan(executable *plan) (*runtimeMetadata, error) {
	target := &runtimeMetadata{Features: requiredFeatures(executable), Kernels: []kernelMetadata{}, Programs: []programMetadata{}}
	for _, kernel := range executable.PhysicalKernels {
		...
		for _, binding := range kernel.Bindings {
			...
			item.Bindings = append(item.Bindings, bindingMetadata{Group: 0, Binding: binding.Binding, Access: access, Type: name, MinimumByteSize: binding.MinimumByteSize, Kind: kind})
		}
	...
	for _, plan := range executable.Programs {
		...
		for _, transient := range plan.Transients {
			description, err := describeTransient(program, transient.Type, transient.Stride, transient.Alignment, transient.MinimumByteSize, transient.Length, transient.Color, transient.FirstStep, transient.LastStep)
			if err != nil {
				return nil, err
			}
			item.Transients = append(item.Transients, description)
		}
		...
		if plan.View != nil {
			...
			item.View, err = describeView(program, step, plan.View.Width, plan.View.Height, plan.View.OutputColor, plan.View.Output, plan.View.Fused)
			if err != nil {
				return nil, err
			}
		}
...
func describeStep(program *ir.Program, kernels []physicalKernel, step step) (stepMetadata, error) {
	...
	out.Parameters, err = describeParameters(program, kernels[step.Kernel].Parameters, step.Parameters)
	return out, err
}
