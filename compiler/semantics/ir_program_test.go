...
func logical(t *testing.T) *ir.Module {
	t.Helper()
	return analyzeSource(t, "flow.tach", `export function fill[i](out: buffer<uint32[]>) { if (i < out.length) { out[i] = i; } }`)
}

func viewLogical(t *testing.T) *ir.Module {
	t.Helper()
	return analyzeSource(t, "view.tach", `
...
func TestConstantArgumentsCloneAndVerifyCanonically(t *testing.T) {
	m := analyzeSource(t, "constant.tach", `
