...
func (e *plan) IndexFunctions() (map[*ir.Function]*ir.Coordinates, map[*ir.Function]*physicalKernel, error) {
	coordinates := map[*ir.Function]*ir.Coordinates{}
	...
	coordinates, err := ir.ResolveFunctionCoordinates(e.KernelModule, coordinates)
	return coordinates, kernels, err
}
...
func chooseWorkgroup(function *ir.Function) ([3]uint32, error) {
	...
	if !valid {
		return size, fmt.Errorf("stage %s workgroup exceeds Vulkan limits", function.Name)
	}
	return size, nil
}
...
func planTransients(program *ir.Program, omitted ir.ResourceID) []transient {
	lifetimes := ir.AnalyzeTransientLifetimes(program, omitted)
	out := make([]transient, len(lifetimes))
	for index, lifetime := range lifetimes {
		resource := program.Resource(lifetime.Resource)
		layout, _ := foundation.LayoutOf(resource.Type)
		out[index] = transient{Type: resource.Type, Stride: layout.Stride, Alignment: layout.Align, MinimumByteSize: layout.Stride, Length: resource.Length, Color: lifetime.Color, FirstStep: lifetime.FirstStep, LastStep: lifetime.LastStep}
	}
	return out
}
...
func projectionKernel() (physicalKernel, error) {
	...
	physical.Bindings = []storageBinding{
		...
		{Buffer: 1, Binding: 1, Access: ir.Mutable, Type: function.BufferParams[1].Type, MinimumByteSize: 4},
	}
	var err error
