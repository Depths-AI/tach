...
func discover(root string) ([]kernel, foundation.Diagnostics) {
	...
	for _, entry := range entries {
		...
		for _, child := range children {
			...
			// DECISION: os.SameFile is portable but O(nÂ²); use platform file IDs if
...
func (p *project) description() ProjectInput {
	input := ProjectInput{Name: p.Manifest.Name, Version: p.Manifest.Version, Package: p.Manifest.JavaScript.Package, Title: p.Manifest.Docs.Title, Summary: p.Manifest.Docs.Summary}
	for _, kernel := range p.Kernels {
		item := KernelInput{Module: kernel.Module, Name: kernel.Name, Identity: kernel.Identity, Documentation: kernel.Documentation}
		for _, declaration := range kernel.Syntax.Decls {
