...
// Result is a complete Pine compilation. Every artifact in Result has already
// passed the validators owned by the Pine compiler.
type Result struct {
	SourceName string
	Module     *ir.Module
	IR         string
...
// Compile runs the complete Pine pipeline: parsing, semantic analysis,
...
func WriteDirectory(result *Result, dir, base string) ([]string, error) {
	...
	artifacts := []struct {
		...
		{base + ".pine.json", result.Metadata},
	}
