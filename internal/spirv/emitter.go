...
// Emit lowers verified Pine IR to a SPIR-V 1.3 compute module and immediately
// parses and validates the produced binary with Pine's own SPIR-V validator.
func Emit(m *ir.Module) ([]byte, error) {
	...
	if err := Validate(out); err != nil {
		return nil, fmt.Errorf("Pine SPIR-V self-validation failed: %w", err)
	}
	return out, nil
}
...
func (b *builder) emitResources() error {
	...
	for i, r := range b.m.Resources {
		...
		emit(&b.debug, OpName, append([]uint32{wrapper}, encodeString("__pine_resource_"+strconv.Itoa(i))...)...)
...
func (b *builder) emitBuiltins() error {
	...
	for _, k := range order {
		...
		emit(&b.debug, OpName, append([]uint32{id}, encodeString("__pine_"+name)...)...)
	}
	return nil
}
...
func (b *builder) emitWorkgroups() error {
	for _, f := range b.m.Functions {
		...
		for i, w := range f.WorkgroupVars {
			...
			emit(&b.debug, OpName, append([]uint32{id}, encodeString("__pine_w_"+f.Name+"_"+strconv.Itoa(i))...)...)
		}
...
func (s *fnEmitter) emitLoop(x *ir.Loop) error {
	...
	// Keep the SPIR-V loop header minimal and structurally stable. Pine's loop
