...
// Instruction is Pine's decoded representation of one SPIR-V instruction.
...
// Module is the binary-level representation consumed by Pine's SPIR-V
...
// outside Pine's owned SPIR-V profile.
func Decode(data []byte) (*Module, error) {
	...
	if words[1] != Version {
		return nil, fmt.Errorf("Pine SPIR-V profile requires version 1.3, got word 0x%08x", words[1])
	}
	if words[3] == 0 {
	...
	for off := 5; off < len(words); {
		...
		if _, ok := opNames[op]; !ok {
			return nil, fmt.Errorf("word %d: opcode %d is outside Pine's SPIR-V profile", off, op)
		}
...
func validateArity(op Op, a []uint32) error {
	...
	case OpDecorate:
		...
		case DecorationArrayStride, DecorationBuiltIn, DecorationBinding, DecorationDescriptorSet:
			return exact(a, 3)
		default:
			return fmt.Errorf("decoration %d is outside Pine's profile", a[1])
		}
	case OpMemberDecorate:
		...
		if a[2] != DecorationOffset {
			return fmt.Errorf("member decoration %d is outside Pine's profile", a[2])
		}
		return exact(a, 4)
...
// literalString decodes a SPIR-V null-terminated UTF-8 byte string. Pine names
...
// Validate owns the complete validity contract for Pine-generated SPIR-V. It
...
func (v *validation) validateLayoutAndDefinitions() error {
	...
	for i, in := range v.m.Instructions {
		...
		case OpCapability:
			...
			if a[0] != CapabilityShader {
				return fmt.Errorf("only Shader capability is valid in Pine profile")
			}
		case OpExtInstImport:
			...
			if name != "GLSL.std.450" {
				return fmt.Errorf("Pine supports only GLSL.std.450 extended instructions, got %q", name)
			}
		...
		case OpMemoryModel:
			...
			if a[0] != AddressingLogical || a[1] != MemoryGLSL450 {
				return fmt.Errorf("Pine requires Logical + GLSL450 memory model")
			}
		case OpEntryPoint:
			if a[0] != ExecutionModelGLCompute {
				return fmt.Errorf("Pine supports GLCompute entry points only")
			}
		...
		case OpExecutionMode:
			if a[1] != ExecutionModeLocalSize {
				return fmt.Errorf("Pine supports LocalSize execution mode only")
			}
		...
		case OpTypeInt:
			if a[1] != 32 || (a[2] != 0 && a[2] != 1) {
				return fmt.Errorf("Pine integer type must be 32-bit signed/unsigned")
			}
			v.types[a[0]] = &typeInfo{kind: typeInt, width: a[1], signed: a[2] == 1}
		case OpTypeFloat:
			if a[1] != 32 {
				return fmt.Errorf("Pine float type must be f32")
			}
			v.types[a[0]] = &typeInfo{kind: typeFloat, width: a[1]}
		case OpTypeVector:
			if a[2] < 2 || a[2] > 4 {
				return fmt.Errorf("Pine vector width must be 2..4")
			}
		...
		case OpTypePointer:
			if a[1] != StorageInput && a[1] != StorageUniform && a[1] != StorageWorkgroup && a[1] != StorageStorageBuffer {
				return fmt.Errorf("pointer %%%d uses storage class %d outside Pine profile", a[0], a[1])
			}
	...
	if caps != 1 {
		return fmt.Errorf("Pine module must declare Shader capability exactly once")
	}
	if memory != 1 {
		return fmt.Errorf("Pine module must declare one memory model")
	}
	return nil
}
	...
	for _, in := range v.m.Instructions {
		...
		case OpLoopMerge:
			...
			if a[2] != LoopControlNone {
				return fmt.Errorf("%s loop control outside Pine profile", ctx)
			}
		case OpSelectionMerge:
			if err := v.requireID(a[0], ctx); err != nil {
				return err
			...
			if a[1] != SelectionControlNone {
				return fmt.Errorf("%s selection control outside Pine profile", ctx)
			}
		case OpBranch:
			if err := v.requireID(a[0], ctx); err != nil {
				return err
...
func (v *validation) validateAtomic(in Instruction) error {
	...
	if scope != wantScope {
		return fmt.Errorf("%s scope=%d, Pine requires %d for storage class %d", ctx, scope, wantScope, pt.storage)
	}
	...
	if sem != MemorySemanticsRelaxed {
		return fmt.Errorf("%s memory semantics=0x%x, Pine atomics require Relaxed", ctx, sem)
	}
...
func (v *validation) validateBarrier(in Instruction) error {
	...
	if sem != wg && sem != storage {
		return fmt.Errorf("%s semantics=0x%x outside Pine barrier profile", ctx, sem)
	}
	return nil
}
...
func (v *validation) validateExtInst(in Instruction) error {
	...
	default:
		return fmt.Errorf("%s GLSL.std.450 instruction %d is outside Pine's profile", ctx, a[3])
	}
	v.valueType[a[1]] = a[0]
	return nil
}
...
func (v *validation) validateBinary(in Instruction) error {
	...
	if in.Op == OpShiftRightLogical || in.Op == OpShiftRightArithmetic || in.Op == OpShiftLeftLogical {
		...
		if in.Op == OpShiftRightArithmetic && !leftBase.signed {
			return fmt.Errorf("%s requires signed shifted value in Pine's profile", ctx)
		}
		if in.Op == OpShiftRightLogical && leftBase.signed {
			return fmt.Errorf("%s requires unsigned shifted value in Pine's profile", ctx)
		}
		v.valueType[a[1]] = a[0]
		return nil
	}
...
func (v *validation) validateDecorationsAndABI() error {
	// Type-level decorations and Pine's host ABI layout.
	...
	for id, t := range v.types {
		if t.kind == typeArray || t.kind == typeRuntimeArray {
			...
			if *d.arrayStride != want {
				return fmt.Errorf("array %%%d ArrayStride=%d, Pine ABI requires %d", id, *d.arrayStride, want)
			}
		}
	...
	for id, storage := range v.globalVars {
		...
		case StorageInput:
			...
			default:
				return fmt.Errorf("Input %%%d uses builtin %d outside Pine profile", id, *d.builtin)
			}
		...
		default:
			return fmt.Errorf("global variable %%%d storage class %d outside Pine profile", id, storage)
		}
	}
	return nil
}
...
func (v *validation) abiOf(id uint32, memo map[uint32]abiLayout, vis map[uint32]bool) (abiLayout, error) {
	...
	case typeStruct:
		...
		for i, mid := range t.members {
			...
			if !ok || got != want {
				return l, fmt.Errorf("struct %%%d member %d Offset=%d, Pine ABI requires %d", id, i, got, want)
			}
			if ml.runtime {
				runtime = true
			} else {
	...
	default:
		return l, fmt.Errorf("type %%%d is not in Pine host ABI", id)
	}
	memo[id] = l
	return l, nil
}
