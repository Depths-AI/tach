...
value-returning helpers may add `returns`. Referenced identifiers are checked.
Use `//` only for local implementation reasoning; block comments do not exist.
See §10: file anatomy,
§11: structured docs, and
§13: comments.

`export` only gates generated JS/TS function exposure. It does not control Tach
...
transient declarations and `run stage(...) over domain;`; they contain no
ordinary control flow and cannot be called from Tach. See
§15: helpers,
§16: stages, and
§18: programs.
...
read. Every `run` is a distinct ordered dispatch; assume no stage fusion. See
§23: size inference,
§24: shapes, and
§25: transients.
...
Generated calls return `ComputeCommand`, not promises or results. Do not mutate
retained object/array parameters before submission, await a command directly,
or submit raw/foreign commands. `repeat` repeats the complete program. Indexed
shorthand also accepts rank-specific `size`. See
§57: commands and
§58: options.

`submit(first, ...rest)` preserves command order and batches work; its promise
ends after preparation/queueing, not completion. Use `idle`, `read`, or scoped
exit only when the CPU needs completion. Device loss requires a new session and
recreated state. Handle `TachError` by `code`, `operation`, message, and cause.
See §59: submission,
§60: completion, and
§61: errors.
...
commands, minimize dispatches, and delay synchronization/readback. Distinguish
...
groups, and real lifecycle contracts. See
