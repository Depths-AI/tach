...
harness, on the other host: do the fourteen public programs in `examples/`
...
`examples.ts` then imports all fourteen public programs from one generated
...
shared counters, 32-bit bit ops, loops and math, binary16 storage and arithmetic,
exact `.length` for direct and struct-tail padding cases, a type imported from
another file, several recipes on one buffer, and `prepare` before `submit`.
...
the selected physical adapter, fourteen-program count, and 72 projected frames.
