...
harness, on the other host: do the sixteen public programs in `examples/`
...
`examples.ts` then imports all sixteen public programs from one generated
...
file, several recipes on one buffer, and `prepare` before `submit`.
...
the selected physical adapter, sixteen-program count, and 72 projected frames.
