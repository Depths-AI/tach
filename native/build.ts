...
const root = new URL("../", import.meta.url), sdk = Deno.env.get("VULKAN_SDK");
...
await Deno.mkdir(new URL("tach-ts/native/", root), { recursive: true });
for (
  const args of [["test", "-race", "-tags", "tachvulkan", "./native"], [
    "build",
    "-tags",
    "tachvulkan",
    "-buildmode=c-shared",
...
await Deno.remove(
  new URL(`tach-ts/native/${name.replace(/\.[^.]+$/u, ".h")}`, root),
).catch((error) => {
