...
async function installCompiler(
  ...
  try {
    ...
    const actual = Array.from(
      new Uint8Array(
        await crypto.subtle.digest("SHA-256", binary.slice().buffer),
      ),
...
function parseRuntime(source: string): RuntimeMetadata {
  ...
  if (
    runtime.schema !== 1 || !Array.isArray(runtime.programs) ||
...
function generatedModule(runtime: RuntimeMetadata): string {
  ...
  let source =
    `// Generated by Tach.\nimport { defineModule as $defineModule } from "@depths/tach/internal";\n\nconst $tach = $defineModule({ ...${definition}, shaders: { web: new URL("./kernel.wgsl", import.meta.url), spirv: new URL("./kernel.spv", import.meta.url) } });\n\n`;
...
function generatedDeclarations(
  ...
  let out =
    '// Generated by Tach. Typed host-independent GPU module.\n\nimport type { CommandOptions as $CommandOptions, ComputeBuffer as $ComputeBuffer, ComputeCommand as $ComputeCommand, LaunchOptions as $LaunchOptions } from "@depths/tach";\n\n';
  for (
...
export async function build(
  ...
  try {
    ...
    await Promise.all([
      Deno.writeTextFile(join(stage, "index.js"), generatedModule(runtime)),
      Deno.writeTextFile(
